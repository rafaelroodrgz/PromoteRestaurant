...
export default function RestaurantsScreen ({ navigation, route }) {
  ...
  const [restaurantPromote, setRestaurantPromote] = useState(null)
  ...
  const renderRestaurant = ({ item }) => {
    return (
      ...
      >
        ...
        {item.promoted &&
        <TextRegular textStyle={styles.promotion }>!En promocion!</TextRegular>}
        ...
        <Pressable
              onPress={() => { setRestaurantPromote(item) }}
              style={({ pressed }) => [
                {
                  backgroundColor: pressed
  ...
  return (
    ...
    />
    <ConfirmationModal
      isVisible={restaurantPromote !== null}
      onCancel={() => setRestaurantPromote(null)}
      onConfirm={() => promoteRestaurant(restaurantPromote)}>
        <TextRegular>The restaurant status will be changed OMGGGG</TextRegular>
        <TextRegular>If the restaurant has null deliveredAt orders, error will appear mai friend.</TextRegular>
    </ConfirmationModal>
    <DeleteModal
